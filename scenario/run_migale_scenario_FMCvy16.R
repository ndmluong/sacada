...
## Scenario Social distancing : conveyor dimension (change position if needed)
Parms_Plant$Objects$cvy$dim.X <- 16
Parms_Plant$Objects$cvy$pos.X <- 0.17

Parms_Plant$Objects$cvy2$dim.X <- 16
Parms_Plant$Objects$cvy2$pos.X <- 0.17

Parms_Plant$Objects$epm1$pos.X <- 0.85

# FUNCTIONS #####
source("functions/functions.R")
# visualize the plant
# f_plotPlant(Plant = f_createPlant(prm = Parms_Plant), prm = Parms_Plant) 
...
## > output file name  ####
output_filename <- paste("simulation_output/", MySeed, "/", "output_FMCvy16_", MySeed,".RData", sep ="")
