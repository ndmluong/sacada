...
## Uncertainty analysis - UA_Vsed_A1 (check the XLSX file)
...
f_sed_time <- function(
  ...
  Vsed <<- matrix(f_Vsed(prm_air),nrow=1) / 7.36 # (m.s-1)  ####################### CHANGE !
...
## > output file name  ####
output_filename <- paste("simulation_output/", MySeed, "/", "output_UA_Vsed_A1_", MySeed,".RData", sep ="")
