...
lapply(MCvy, function(x) {
  ...
  rbindlist() %>%
  mutate(., MaskPercentage = Mask * 100) %>%
...
AverageDailySurfacesContaRatio_clus1$MaskPercentage <- as.factor(AverageDailySurfacesContaRatio_clus1$MaskPercentage)
...
AverageDailySurfacesContaRatio_clus0$MaskPercentage <- as.factor(AverageDailySurfacesContaRatio_clus0$MaskPercentage)
...
AverageDailyFoodContaRatio_clus1$MaskPercentage <- as.factor(AverageDailyFoodContaRatio_clus1$MaskPercentage)
...
AverageDailyFoodContaRatio_clus0$MaskPercentage <- as.factor(AverageDailyFoodContaRatio_clus0$MaskPercentage)
...
gMCvy_ClusterProb <- ggplot(data = SMR_MCvy) +
  ...
  scale_color_viridis(discrete = TRUE, option = "plasma", name = "Distance\nbetween workers", begin = 0, end = 0.8,
                      labels = c("1 m ", "1.3 m", "1.6 m", "2 m")) +
  scale_x_continuous(breaks = unique(Mask)) +
  xlab("Mask-wearing (%)") + ylab("Cluster-occurrence probability (%)")
gMCvy_ClusterProb
ggsave("simulation_output/plot/gMCvy_ClusterProb.pdf", gMCvy_ClusterProb, width = 6.5, height = 4.5)

## Peak_clus ####
# SMR_MCvy$Mask <- as.factor(SMR_MCvy$Mask)

gMCvy_PeakClus <- ggplot(data = SMR_MCvy) +
  theme(axis.ticks=element_blank(),
        legend.position = "right",
        legend.title.align = 0.5,
        panel.background=element_rect(fill="white"),
...
gMCvy_PeakClus
ggsave("simulation_output/plot/gMCvy_PeakClus.pdf",
       gMCvy_PeakClus,
...
g_MCvy_DailySurfacesConta_clus1 <- ggboxplot(
  data = AverageDailySurfacesContaRatio_clus1, x = 'MaskPercentage', y = 'SurfacesContaRatioPercentage', fill = 'Cvy') +
  theme(axis.ticks=element_blank(),
        legend.position = "bottom",
        panel.background=element_rect(fill="white"),
        plot.title = element_text(face = "bold", size=14),
  ...
  scale_fill_viridis(discrete = TRUE, option = "plasma", name = "Distance\nbetween workers", begin = 0, end = 0.8,
                     labels = c("1 m ", "1.3 m", "1.6 m", "2 m")) +
  xlab("Mask-wearing (%)") + ylab("Daily ratio of surface contamination\nAverage values across a 42-day period (%)") +
...
g_MCvy_DailySurfacesConta_clus0 <- ggboxplot(
  data = AverageDailySurfacesContaRatio_clus0, x = 'MaskPercentage', y = 'SurfacesContaRatioPercentage', fill = 'Cvy') +
  theme(axis.ticks=element_blank(),
        legend.position = "bottom",
        panel.background=element_rect(fill="white"),
        plot.title = element_text(face = "bold", size=14),
  ...
  scale_fill_viridis(discrete = TRUE, option = "plasma", name = "Distance\nbetween workers", begin = 0, end = 0.8,
                     labels = c("1 m ", "1.3 m", "1.6 m", "2 m")) +
  xlab("Mask-wearing (%)") + ylab("Daily ratio of surface contamination\nAverage values across a 42-day period (%)") +
  labs(subtitle = "Situations with fewer than 25 infections")
g_MCvy_DailySurfacesConta_clus0

gMCvy_DailySurfacesConta <- ggarrange(g_MCvy_DailySurfacesConta_clus1 + rremove("xlab") + rremove("ylab"), 
                                      g_MCvy_DailySurfacesConta_clus0 + rremove("xlab") + rremove("ylab"), 
                                      ncol = 2,
                                      common.legend = TRUE, legend = "right") %>%
  annotate_figure(top = grid::textGrob("Surface contamination (%)", gp = grid::gpar(cex = 2)),
                  left = grid::textGrob("Daily ratio of surface contamination\nAverage values across a 42-day period (%)", rot = 90, gp = grid::gpar(cex = 1.3)),
                  bottom = grid::textGrob("Mask-wearing (%)", gp = grid::gpar(cex = 1.3)))

ggsave("simulation_output/plot/gMCvy_DailySurfacesConta.pdf",
       gMCvy_DailySurfacesConta, width = 12.5, height = 6.5)
...
g_MCvy_DailyFoodConta_clus1 <- ggboxplot(
  data = AverageDailyFoodContaRatio_clus1, x = 'MaskPercentage', y = 'FoodContaRatioPercentage', fill = 'Cvy') +
  theme(axis.ticks=element_blank(),
        legend.position = "bottom",
        panel.background=element_rect(fill="white"),
        plot.title = element_text(face = "bold", size=14),
  ...
  scale_fill_viridis(discrete = TRUE, option = "plasma", name = "Distance\nbetween workers", begin = 0, end = 0.8,
                     labels = c("1 m ", "1.3 m", "1.6 m", "2 m")) +
  xlab("Mask-wearing (%)") + ylab("Daily ratio of food contamination\nAverage values across a 42-day period (%)") +
  labs(subtitle = "Situations with more than 25 infections")
...
gMCvy_DailyFoodConta <- ggarrange(g_MCvy_DailyFoodConta_clus1 + rremove("xlab") + rremove("ylab"), 
...
ggsave("simulation_output/plot/gMCvy_DailyFoodConta.pdf",
       gMCvy_DailyFoodConta, width = 12.5, height = 6.5)
...
## Production losses ####
situation_linetype = c("Cluster situations" = 1, "Non-cluster situations" = 2)

loss_clus <- rbind(data.frame(loss_clus1, clus = rep(1, nrow(loss_clus1))),
                   data.frame(loss_clus0, clus = rep(0, nrow(loss_clus0))))
loss_clus$clus <- factor(loss_clus$clus, levels = unique(loss_clus$clus))
...
gMCvy_loss <- ggplot() +
  theme(axis.ticks=element_blank(),
        ...
        panel.grid.minor.y=element_line(colour="white"),
        panel.grid.minor.x=element_blank()) +
  # stat_summary(data = loss_clus1, mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy), fun = mean, geom = "line", linewidth = 1.2) +
  stat_summary(data = loss_clus1, mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy), fun = mean, geom = "point", size = 2) +
  # stat_summary(data = loss_clus0, mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy), fun = mean, geom = "line", alpha = 0.7, linewidth = 0.5, linetype = 2) +
  stat_summary(data = loss_clus0, mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy), fun = mean, geom = "point", size = 2, alpha = 0.7) +
  stat_summary(data = subset(loss_clus, clus == 1),
               mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy, linetype = clus, linewidth = clus), alpha = 1,  fun = mean, geom = "line") +
  stat_summary(data = subset(loss_clus, clus == 0),
               mapping = aes(x = Mask*100, y = Prod, group = Cvy, colour = Cvy, linetype = clus, linewidth = clus), alpha = 0.7, fun = mean, geom = "line") +
  scale_colour_viridis(discrete = TRUE, option = "plasma", name = "Distance\nbetween workers", begin = 0, end = 0.8,
                       labels = c("1 m ", "1.3 m", "1.6 m", "2 m")) +
  scale_linetype_manual(name = "", values = c("1" = 1, "0" = 2), breaks = c("1", "0"), labels = c("Cluster situations", "Non-cluster situations")) +
  scale_linewidth_manual(name = "", values = c("1" = 1.2, "0" = 0.5), breaks = c("1", "0"), labels = c("Cluster situations", "Non-cluster situations")) +
  scale_alpha_manual(name = "", values = c(1, 0.7), label = c("Cluster situations", "Non-cluster situations"), guide = F) +
  scale_x_continuous(breaks = unique(Mask)) +
  xlab("Mask-wearing (%)") + ylab("Production capacity (%)")
gMCvy_loss

ggsave("simulation_output/plot/gMCvy_loss.pdf",
       gMCvy_loss, width = 6.5, height = 4.5)
