...
raw_Conv <- f_checkout(scenario = "Conv")
fullConv <- f_summaryOutput(rawoutput = raw_Conv)

# Clear environment
gdata::keep(raw_Conv, fullConv, sure = TRUE)

save.image("simulation_output/202209_FullConvergenceCheck_FullData.RData")
...
# Load all summaries #####
load("simulation_output/202209_FullConvergenceCheck_Summary.RData")
...
FPS = fullConv$FPS
SS = fullConv$SS
MyPlant = fullConv$MyPlant
Parms_Conta = fullConv$Parms_Conta
Parms_workers = fullConv$Parms_Workers


## All indicators ####
f_plotConvAllIndicators(IL = IL,
                        IS = IS,
                        FPS = FPS,
                        SS = SS,
                        plant = MyPlant,
                        ...
                        keptsim = c(50, 100, 150, 200, 500),
                        detection = 5,
                        CVmax = 0.2,
                        resampling = 20,
                        plotseed = 407) -> g_ConvergenceAllIndicators


ggsave(filename="simulation_output/plot/g_ConvergenceAllIndicators_full20rs.pdf", 
       plot = g_ConvergenceAllIndicators, 
       device = cairo_pdf, width = 500, height = 297, units = "mm")
