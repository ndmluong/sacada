...
f_allocateCarcassTimeIndex <- function(
  ...
  ## Total daily number of carcasses to be processed depending on the total active number of cutter a
...
##### en cours: f_initFood() FUNCTION TO INITIALISE THE AGENTS "FOOD PORTIONS" #####
f_initFood <- function(
  ...
  #### BEGIN OF FUNCTION
  writeLines("==================== Initializing Food Portions ================================")
  ...
  ## create unique ID for each portion based on the type of meat, the day, the carcass number and the portion number
  writeLines(">>> Assigning food portions ID ")
  expand.grid(str_pad(day, width = 2, pad = "0"), # day number
              str_pad(1:nrow(c_alloc), width = 4, pad = "0"), # carcass number
              str_pad(1:N_FP_perCarcass, width = 3, pad = "0")) %>% # pieces number (per carcass)
    as.data.frame(.) %>%
  ...
  writeLines(">>> Processing time indexes for each food portion")
  apply(c_alloc, 1, FUN = function(x) {
    carcass_x <- unique_ID[substr(unique_ID,6,9) == x[["carcass_ID"]] ]
    cut_t_ind_x_vec <- x[["cut_t_ind"]]:x[["cut_t_ind_end"]]
    expand.grid(carcass_x, cut_t_ind_x_vec) %>%
      as.data.frame(.) -> FP_x
  }) %>%
  ...
  writeLines(">>> Food portions initialized: carcasses workflow & food portions attributes")
