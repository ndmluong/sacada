...
fomites), the contamination throught the exposition outside the factory
and for employees leaving in community, the intra-group transmission
...
Here are the elements taken into account for six rooms in the whole
plant (cf. Figure \@ref(fig:FIG-map-plant)) *(associated R script:
parameters_plant.R)* :
...
Each room needs to be positioned on the entire workshop grid. The
cutting room is the main build by subtracting all other rooms. The
position of the door is also mentioned with four variables : "top",
"bottom", "left" or "right" (Table
...
For this purpose each room has to be defined. Two elements are taken
into account
...
The objects must be defined. They are positioned in the model on the X
and Y axis, together with the dimensions.

As illustrated in Figure \@ref(fig:FIG-map-plant-object), we implemented
two types of objects: two conveyors, that allows the pieces of meat to
be transported from one point to another, and an equipment that can be
shared by several operators, such as a peeler. The parameters associated
with these objects are gathered in Table
\@ref(tab:TAB-plant-object-parameters).
