...
author:
  - name: Ngoc-Du Martin Luong
    ...
    email: ngoc-du.luong@anses.fr
  - name: Laurent Guillier
    orcid: 0000-0002-7867-2937
    email: laurent.guillier@anses.fr
...
The outline of the agent-based modelling workflow developed in this study is schematized in @fig-abm-outline.
...
```{r}
#| label: tbl-plant-object-parameters
#| tbl-cap: 'Parameters characterizing the different objects in the plant.'
read.csv("csv/parameters_object_plant.csv",sep=";",header = T) %>%
  knitr::kable(., col.names = gsub("[.]", " ", names(.))) %>%
...
```{r}
#| label: tbl-workers-parameters
#| tbl-cap: 'Parameters characterizing the different types of worker.'
read.csv("csv/parameters_worker.csv",sep=";",header = T) %>%
  as.data.frame %>%
  dplyr::select(., -6) %>%
  knitr::kable(., col.names = gsub("[.]", " ", names(.))) %>% 
  kable_styling('striped', font_size = 11) %>%
...
#| fig-align: 'center'
#| out-width: "65%"
#| fig-cap: "Representation of the stage of infection for SARS-CoV2"
...
```{r}
#| label: tbl-infection-duration
#| tbl-cap:  'Set of parameters used for charaterizing the durations of the SARS-CoV2 infection.'
read.csv("csv/parameters_duration.csv",sep=";",header = T) %>%
  as.data.frame %>%
  dplyr::select(., -6) %>%
  knitr::kable(., col.names = gsub("[.]", " ", names(.))) %>%
  kable_styling('striped', font_size = 11)
...
```{r}
#| label: tbl-load-VoC
#| tbl-cap: "Parameters characterizing inter-individual variability of viral loads (expressed in log10 copy/ml) in sputum."
read.csv("csv/parameters_vload.csv",sep=";",header = T) %>%
  as.data.frame() %>%
  dplyr::select(., -7) %>%
  knitr::kable(col.names = gsub("[.]", " ", names(.)),
...
The secondary attack rate (SAR) parameter used to estimate the probability of contraction SARS-CoV-2 for the workers having community activities outside the plant such as co-living. The SAR of the original and *alpha* VoCs has been found by @Fung2021 to be equal to 17.1% (95%CI 13-21%). According to @Singanayagam2021, the SAR was found to be 38% for *delta* variant (with 95%CI 24-53%) in case of unvaccinated people, and 25% (with 95%CI 18-33%) for vaccinated people. The SAR for *omicron* is respectively 1.2 for vaccinated and 2.6 times more important than for delta [@Lyngse2021]. These different SAR parameter values included in the model are shown in Table \@ref(tab:TAB-SAR).

```{r}
#| label: tbl-SAR
#| tbl-cap: 'Secondary attack rates in household for different variants of concern and vaccination statuses.'

read.csv("csv/parameters_sar.csv",sep=";",header = T) %>%
  as.data.frame() %>%
  dplyr::select(., -6) %>%
  knitr::kable(.,
...
The occurrence frequency of the above respiratory activities during a time step varied from one worker to another, depending on their working activities as well as their infection phases. For example, [Birring2006 and Hsu1994]{.underline} suggested average coughing frequencies at 2 coughs per hour for healthy patients and 47 coughs per hour for symptomatic patients. Such variability between infection statuses was also suggested for the sneezing frequencies ([Hansen&Mygind2002, Musch2021]{.underline}). These SARS-CoV-2 symptoms frequencies $f_{cough/sneeze}$ (expressed in $\#.min^{-1}$) included in the model are gathered in @tbl-air-freq.

```{r}
#| label: tbl-air-freq
#| tbl-cap: 'Frequency of different SARS-CoV-2 symptoms (expressed as the number of occurrences per minutes).'
...
## Distribution of the projected droplets {#sec-distribution-of-the-projected-droplets}
